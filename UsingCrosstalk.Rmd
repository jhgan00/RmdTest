...
Crosstalk 하나의 Rmarkdown 문서 혹은 Shiny 앱에서 여러 위젯의 연동 기능을 제공합니다(아직은 Crosstalk과 연동 가능한 HTML 위젯은 제한적입니다). 이번 장에서는 Rmarkdown과 `d3scatter`, `leaflet` 패키지를 통해서 실습을 진행합니다. 먼저 깃허브에서 `crosstalk`, `d3scatter`, `leaflet` 패키지를 다운로드하고 불러옵니다.
