...
이번 장에서는 R 마크다운 문서에 샤이니 요소들을 포함하는 것을 넘어서, 하나의 샤이니 앱을 개발하는 것을 목표로 합니다. 앞서 언급했듯이 샤이니는 `ui`와 `server`의 두 부분으로 나눠집니다. 아래는 샤이니의 구조를 최소한의 코드로 표현한 앱입니다. `ui`와  `server`를 만든 후 `shinyApp()` 함수에 `ui`와 `server`를 넣어주면 됩니다. 새로운 R 스크립트를 생성하고, 다음 코드를 실행해보시기 바랍니다. 앱에는 아무 것도 출력되지 않지만, 앱은 정상적으로 작동하고 있습니다.
